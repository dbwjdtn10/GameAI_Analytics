...
> 📸 **준비 중** — Streamlit 대시보드 스크린샷을 추가할 예정입니다.
>
> 추가 예정 화면: Overview KPI 패널, ROC Curve 비교, SHAP Feature Importance, 유저 세그먼트 분포, What-If PDP 분석, Grafana 모니터링 대시보드
...
### 잘한 점
- **MLOps 풀셋 구축**: MLflow + DVC + Airflow + Prometheus/Grafana까지 모델 운영 전체 사이클을 자동화한 것이 가장 큰 성과입니다. 특히 Airflow DAG의 조건부 재학습(drift ratio > 30%일 때만 학습)은 불필요한 컴퓨팅 비용을 절감합니다.
- **Model Regression Gate**: CI에서 AUC-ROC ≥ 0.90, Accuracy ≥ 0.85 성능 게이트를 설정하여, 성능이 떨어지는 코드가 병합되지 않도록 자동화한 점이 프로덕션 품질 유지에 핵심적이었습니다.
- **Feature Store 패턴**: Training-Serving Skew를 경량 Feature Store로 방지한 것은 실무에서도 자주 만나는 문제를 해결한 경험입니다.

### 개선이 필요한 점
- **Model Card 부재**: 모델의 가정, 한계, 공정성에 대한 공식 문서가 없습니다. 특히 연령/성별에 따른 편향 분석이 필요합니다.
- **A/B 테스트 프레임워크**: 리텐션 전략의 효과를 측정하는 통계적 A/B 테스트(p-value, CI) 프레임워크가 아직 미구현입니다.
- **실제 게임 데이터 적용**: 현재 Kaggle 공개 데이터셋 기반이라, 실제 게임의 로그 데이터에 적용하면 피처 엔지니어링이 크게 달라질 수 있습니다.
- **실시간 스트리밍 파이프라인**: 현재 배치 기반 재학습이지만, Kafka/Flink 기반 실시간 이벤트 처리로 확장하면 즉각적인 이탈 경고가 가능합니다.
- **Privacy 강화**: 합성 데이터 생성 시 Differential Privacy 적용, 실제 유저 데이터에 대한 익명화 처리가 필요합니다.
